...
divylimsrange <- abs(c(min(na.omit(newdmslat))-max(na.omit(newdmslat))))
    ...
    ggplot(sf_oz, aes(fill="white")) + geom_sf() +
      ...
      geom_point(data = data11, mapping = aes(x = long, y = lat, color = exp_het), size=8, alpha=0.9) +
    ...
    ggsave(paste0(outputloc, "/diversity/", species, "Expected_Het_Heatmap.tiff"), width = 10, height = 12, dpi = 300, units = "in")
    ...
    ggplot(sf_oz, aes(fill="white")) + geom_sf() +
      ...
      theme_void() + geom_sf() +
      xlim(divxlims) + ylim(divylims)+
      geom_point(data = data11, mapping = aes(x = long, y = lat, color = obs_het), size=8, alpha=0.9) +
      scale_color_gradient(low="yellow", high="red", name="Obs Het") +
      geom_point(data=coordinate_cities, mapping = aes(x = long, y = lat), size=1)+
      geom_text(data=coordinate_cities, mapping = aes(x = long, y = lat, label=City),hjust=0,nudge_y=0.03, size=3)


    ggsave(paste0(outputloc, "/diversity/", species, "Observed_Het_Heatmap.tiff"), width = 10, height = 12, dpi = 300, units = "in")
    ...
    ggplot(sf_oz, aes(fill="white")) + geom_sf() +
      ...
      theme_void() + geom_sf() +
      xlim(divxlims) + ylim(divylims)+
      geom_point(data = data11, mapping = aes(x = long, y = lat, color = ar), size=8, alpha=0.9) +
      scale_color_gradient(low="yellow", high="red", name="ar") +
      geom_point(data=coordinate_cities, mapping = aes(x = long, y = lat), size=1)+
      geom_text(data=coordinate_cities, mapping = aes(x = long, y = lat, label=City),hjust=0,nudge_y=0.03, size=3)


    ggsave(paste0(outputloc, "/diversity/", species, "Allelic_Richness_Heatmap.tiff"), width = 10, height = 12, dpi = 300, units = "in")
...
loc_diversity <- dir(path = outputloc, pattern = "divntab.tiff$", full.names = TRUE, recursive = TRUE) 
loc_exphetdiv <- dir(path = outputloc, "/diversity/", pattern = "Expected_Het_Heatmap.tiff$", full.names = TRUE, recursive = TRUE) 
loc_obshetdiv <- dir(path = outputloc, "/diversity/", pattern = "Observed_Het_Heatmap.tiff$", full.names = TRUE, recursive = TRUE) 
loc_ardiv <- dir(path = outputloc, "/diversity/", pattern = "Allelic_Richness_Heatmap.tiff$", full.names = TRUE, recursive = TRUE) 
...
locs <- c(loc_QC,loc_DistributionMap,loc_PCA_wMap,loc_PCA_Lat,loc_PCA_1_2,loc_PCA_Combined, loc_PCA_Centroid,loc_PCA_Centroidmap,loc_LEAbar, loc_diversity,loc_exphetdiv,loc_obshetdiv, loc_ardiv, loc_Fst,loc_heatmapsFst,loc_Fstq,loc_heatmapsFstq,loc_indivFstback2back, loc_NJTree, loc_Kinship, loc_alleleprop)
